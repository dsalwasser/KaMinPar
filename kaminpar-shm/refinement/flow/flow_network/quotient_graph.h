...
public:
  ...
  using CutEdges = tbb::concurrent_vector<GraphEdge>;
...
public:
  ...
  QuotientGraph &operator=(QuotientGraph &&) noexcept = delete;
  ...
  [[nodiscard]] bool has_edge(const BlockID b1, const BlockID b2) const {
    const Edge &quotient_edge = edge(b1, b2);
    return quotient_edge.cut_weight > 0;
  }

  [[nodiscard]] EdgeWeight edge_weight(const BlockID b1, const BlockID b2) const {
    const Edge &quotient_edge = edge(b1, b2);
    return quotient_edge.cut_weight;
  }

  [[nodiscard]] CutEdges &cut_edges(const BlockID b1, const BlockID b2) const {
    const Edge &quotient_edge = edge(b1, b2);
